...
export async function POST(request: NextRequest) {
  try {
    ...
    const { propertyId, unitNumber, type } = body

    if (!propertyId || !unitNumber || !type) {
      return NextResponse.json({ error: 'Property, unit number and type are required' }, { status: 400 })
    }
    ...
    if (error) {
      if (error.code === '23505') {
        return NextResponse.json({ error: 'This unit number already exists in the selected property' }, { status: 409 })
      }
